...
function locDefineEn() {
    ...
    t[$ "card.name.physicalDamageStunChanceSingleTarget"] = "Stunning Blow"
    t[$ "card.name.physicalDamageStunChanceMultiTarget"] = "Stunning Blow (Group)"
    t[$ "card.name.physicalDamageBleedChanceSingleTarget"] = "Laceration"
    t[$ "card.name.physicalDamageBleedChanceMultiTarget"] = "Laceration (Group)"
    ...
    t[$ "card.name.physicalDamageWeakenChanceSingleTarget"] = "Weakening Blow"
    t[$ "card.name.physicalDamageWeakenChanceMultiTarget"] = "Weakening Blow (Group)"
    t[$ "card.name.physicalDamageVampChanceSingleTarget"] = "Draining Strike"
    t[$ "card.name.physicalDamageVampChanceMultiTarget"] = "Draining Strike (Group)"
    t[$ "card.name.magicalDamageSingleTarget"] = "Star Bolt"
    t[$ "card.name.magicalDamageMultipleTarget"] = "Star Bolt (Group)"
    t[$ "card.name.magicalDamageStunChanceSingleTarget"] = "Lightning"
    t[$ "card.name.magicalDamageStunChanceMultiTarget"] = "Lightning (Group)"
    t[$ "card.name.magicalDamageBurnChanceSingleTarget"] = "Fireball"
    t[$ "card.name.magicalDamageBurnChanceMultiTarget"] = "Fireball (Group)"
    t[$ "card.name.magicalDamageFreezeChanceSingleTarget"] = "Frost"
    t[$ "card.name.magicalDamageFreezeChanceMultiTarget"] = "Frost (Group)"
    t[$ "card.name.buffPhysicalDamageSingleTarget"] = "Empower"
    t[$ "card.name.buffMagicalDamageSingleTarget"] = "Focus"
    t[$ "card.name.buffAnyDamageMultiTarget"] = "Battle Cry"
    t[$ "card.name.buffPhysicalProtectionSingleTarget"] = "Guard"
    t[$ "card.name.buffMagicalProtectionSingleTarget"] = "Ward"
    t[$ "card.name.buffAnyProtectionMultiTarget"] = "Shield Wall"
    t[$ "card.name.debuffPhysicalDamageSingleTarget"] = "Weaken"
    t[$ "card.name.debuffMagicalDamageSingleTarget"] = "Dampen"
    t[$ "card.name.debuffPhysicalProtectionSingleTarget"] = "Expose"
    t[$ "card.name.debuffMagicalProtectionSingleTarget"] = "Disrupt"
    ...
    t[$ "card.name.ignoreWeaknessSingleTarget"] = "Resolve"
    t[$ "card.name.instantHealSingleTarget"] = "Heal"
    t[$ "card.name.instantHealMultiTarget"] = "Heal (Group)"
    t[$ "card.name.overtimeHealSingleTarget"] = "Regeneration"
    t[$ "card.name.instantManaGainSingleTarget"] = "Mana"
    t[$ "card.name.instantManaGainMultiTarget"] = "Mana (Group)"
    t[$ "card.name.overtimeManaGainSingleTarget"] = "Mana Flow"
    t[$ "card.name.copyNextPlayedCard"] = "Echo"
    ...
    t[$ "card.name.bossClone"] = "Mirror Legion"
...
function locDefineRu() {
    ...
    t[$ "effect.ShuffleDeck"] = "Перемешать"
    ...
    t[$ "unit.Viv"] = "Вив"
    t[$ "unit.Safar"] = "Сафар"
    t[$ "unit.CrackerNut"] = "Орешек"
    t[$ "unit.Leaf"] = "Листик"
    t[$ "unit.Mushroom"] = "Грибок"
    t[$ "unit.Flower"] = "Цветок"
    t[$ "unit.Puppet Master"] = "Кукловод"
    t[$ "unit.ignitePrefix"] = "Пылающий "
    ...
    t[$ "dlg.tut.ow1"] = "Стой - дикий Старриор! Хороший шанс научиться сражаться."
    t[$ "dlg.tut.ow2"] = "Не переживай, с картами всё просто, стоит только освоиться."
    t[$ "dlg.tut.ow3"] = "Идём. Я всё объясню, как только начнётся бой."
    t[$ "dlg.tut.deck1"] = "Отличная работа! Теперь соберём твою колоду на будущее."
    ...
    t[$ "dlg.tut.battle2"] = "Это ИНФО - осмотри характеристики врага перед тем, как действовать."
    t[$ "dlg.tut.battle3"] = "Это МЕШАТЬ - пересдаёт всю твою руку на этот ход."
    t[$ "dlg.tut.battle4"] = "Вот и всё. Теперь одолей этого Старриора сам. Удачи!"
    ...
    t[$ "dlg.tut.db3"] = "Вот и всё - собирай колоду как хочешь. Нажми Tab, чтобы закрыть."
    ...
    t[$ "card.name.physicalDamageMultipleTarget"] = "Физ. удар (группа)"
    ...
    t[$ "card.name.physicalDamageBleedChanceSingleTarget"] = "Рассечение"
    t[$ "card.name.physicalDamageBleedChanceMultiTarget"] = "Рассечение (группа)"
    ...
    t[$ "card.name.physicalDamageVampChanceSingleTarget"] = "Вытягивающий удар"
    t[$ "card.name.physicalDamageVampChanceMultiTarget"] = "Вытягивающий удар (группа)"
    t[$ "card.name.magicalDamageSingleTarget"] = "Звёздный залп"
    t[$ "card.name.magicalDamageMultipleTarget"] = "Звёздный залп (группа)"
    t[$ "card.name.magicalDamageStunChanceSingleTarget"] = "Молния"
    t[$ "card.name.magicalDamageStunChanceMultiTarget"] = "Молния (группа)"
    t[$ "card.name.magicalDamageBurnChanceSingleTarget"] = "Огненный шар"
    t[$ "card.name.magicalDamageBurnChanceMultiTarget"] = "Огненный шар (группа)"
    t[$ "card.name.magicalDamageFreezeChanceSingleTarget"] = "Мороз"
    t[$ "card.name.magicalDamageFreezeChanceMultiTarget"] = "Мороз (группа)"
    t[$ "card.name.buffPhysicalDamageSingleTarget"] = "Усиление силы"
    t[$ "card.name.buffMagicalDamageSingleTarget"] = "Усиление магии"
    t[$ "card.name.buffAnyDamageMultiTarget"] = "Боевой клич"
    t[$ "card.name.buffPhysicalProtectionSingleTarget"] = "Защита"
    t[$ "card.name.buffMagicalProtectionSingleTarget"] = "Магический щит"
    t[$ "card.name.buffAnyProtectionMultiTarget"] = "Стена щитов"
    t[$ "card.name.debuffPhysicalDamageSingleTarget"] = "Ослабить силу"
    t[$ "card.name.debuffMagicalDamageSingleTarget"] = "Ослабить магию"
    t[$ "card.name.debuffPhysicalProtectionSingleTarget"] = "Пробить защиту"
    t[$ "card.name.debuffMagicalProtectionSingleTarget"] = "Пробить щит"
    t[$ "card.name.weaknessMagicalDamageSingleTarget"] = "Магическая уязвимость"
    t[$ "card.name.weaknessPhysicalDamageSingleTarget"] = "Физическая уязвимость"
    t[$ "card.name.ignoreWeaknessSingleTarget"] = "Стойкость"
    t[$ "card.name.instantHealSingleTarget"] = "Лечение"
    t[$ "card.name.instantHealMultiTarget"] = "Лечение (группа)"
    t[$ "card.name.overtimeHealSingleTarget"] = "Регенерация"
    t[$ "card.name.instantManaGainSingleTarget"] = "Мана"
    t[$ "card.name.instantManaGainMultiTarget"] = "Мана (группа)"
    t[$ "card.name.overtimeManaGainSingleTarget"] = "Поток маны"
    t[$ "card.name.copyNextPlayedCard"] = "Эхо"
    t[$ "card.name.addEnergy"] = "Доп. энергия"
    t[$ "card.name.shuffleDeck"] = "Перемешать"
    t[$ "card.name.stealCard"] = "Кража"
    ...
    t[$ "card.name.summonAttackPuppet"] = "Марионетка-боец"
    t[$ "card.name.summonMagicPuppet"] = "Марионетка-маг"
    t[$ "card.name.summonHealPuppet"] = "Марионетка-лекарь"
    t[$ "card.name.summonBuffPuppet"] = "Марионетка-жрец"
    t[$ "card.name.bossClone"] = "Зеркальный легион"
    ...
    t[$ "card.desc.summonAttackPuppet"] = "создаёт слабую марионетку"
    t[$ "card.desc.summonMagicPuppet"] = "создаёт слабую марионетку"
    t[$ "card.desc.summonHealPuppet"] = "создаёт слабую марионетку"
    t[$ "card.desc.summonBuffPuppet"] = "создаёт слабую марионетку"
