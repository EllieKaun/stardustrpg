function stepVictoryScreen() {
    ...
    if (count == 0) {   // empty pool fallback
...
// Reuses your battle card-info layout
function drawRewardDescription(card, px, py, pw, ph) {
    draw_sprite_stretched(sprCardDesk, 0, px, py, pw, ph);   // red panel to match
    ...
    if (effectStr != "") { draw_text(ix, cy, "Effects: " + effectStr); cy += lh; }
...
function drawGameOverScreen() {
    ...
    draw_set_halign(fa_center);
    draw_set_valign(fa_middle);
    draw_text(sw / 2, floor(sh * 0.30), "GAME OVER");
