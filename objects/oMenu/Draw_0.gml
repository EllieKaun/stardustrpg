...
for(l = 0; l < (visibleOptionsMax + _desc); l++){
	...
	if((l == 0) && (_desc)){
		...
		if(options[_optionsToShow][3] == false){
...
draw_sprite(sPointer, 0, x + xmargin - 1, y + ymargin + ((hover - _scrollPush) * heightLine) + 7);

if((visibleOptionsMax < array_length(options)) && (hover < array_length(options) -1)){
	draw_sprite(sArrow, 0, x + widthFull * 0.5, y + heightFull - 7 );
}